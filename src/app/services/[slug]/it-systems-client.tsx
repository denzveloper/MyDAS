...
export default function ITSystemsClient({ service }: ITSystemsClientProps) {
  ...
  const stats: StatItem[] = [
    ...
    { value: '24/7', label: 'Support & Monitoring', icon: Clock },
    ...
    <div className="min-h-screen bg-gray-950 text-gray-200">
          ...
          >
            <div className="md:max-w-2xl">
              ...
              <div className="flex flex-wrap gap-3">
                <span className="bg-blue-400/20 text-blue-400 px-3 py-1 rounded-full text-sm font-medium">Infrastructure</span>
                <span className="bg-blue-400/20 text-blue-400 px-3 py-1 rounded-full text-sm font-medium">Security</span>
                <span className="bg-blue-400/20 text-blue-400 px-3 py-1 rounded-full text-sm font-medium">Cloud Solutions</span>
              </div>
            </div>
            <motion.div 
              variants={scaleIn}
              ...
              whileInView="show"
              viewport={{ once: true, amount: 0.3 }}
              className="hidden md:flex rounded-lg bg-gradient-to-br from-blue-400/30 to-blue-500/20 p-8"
            >
          ...
          >
            {stats.map((stat, index) => (
              <motion.div key={index} variants={item}>
                <Card className="bg-gray-800 border-gray-700 hover:border-blue-400/50 transition-all hover:shadow-lg hover:shadow-blue-400/10">
                  <CardHeader className="flex flex-row items-center justify-between pb-2">
                    <CardTitle className="text-white text-2xl font-bold">{stat.value}</CardTitle>
                    <stat.icon className="h-8 w-8 text-blue-400 opacity-80" />
                  </CardHeader>
                  <CardContent>
                  ...
                  <Accordion type="single" collapsible className="w-full">
                    {service.features.map((feature, index) => (
                      <AccordionItem key={index} value={`feature-${index}`} className="border-gray-700">
                        <AccordionTrigger className="text-lg text-white hover:text-blue-400 transition-colors py-4">
                          {feature.split(':')[0]}
                        </AccordionTrigger>
                ...
                <div className="space-y-12">
                  {service.process.map((step, index) => (
                    <div key={index} className="relative">
                      {/* Process step number */}
                      <div className="absolute -left-4 -top-4 h-12 w-12 rounded-full bg-blue-400 flex items-center justify-center text-gray-900 font-bold text-xl">
                        {index + 1}
                      </div>
                      
                      <div className="ml-8 pl-4 border-l-2 border-blue-400/30">
      ...
      <div className="py-12 bg-gray-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-bold mb-6 text-white">Ready to Upgrade Your IT Systems?</h2>
            <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto">Let us help you build robust, secure, and scalable IT infrastructure for your business.</p>
            <Link 
              href="/contact" 
              className="inline-block bg-blue-400 hover:bg-blue-500 text-gray-900 font-semibold px-8 py-3 rounded-lg text-lg transition-colors"
            >
              Get In Touch
            </Link>
          </motion.div>
